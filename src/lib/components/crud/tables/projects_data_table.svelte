<script lang="ts">
	import type { Project } from '$lib/server/schemas';
	...
	const projects_table = createTable(readable(projects), {
		page: addPagination(),
		sort: addSortBy(),
	...
	const hidableCols = ['name', 'deadline', 'description'];

	const columns = projects_table.createColumns([
		projects_table.column({
			accessor: 'id',
	...
	const { headerRows, pageRows, tableAttrs, tableBodyAttrs, pluginStates, flatColumns, rows } =
		projects_table.createViewModel(columns);
							...
							{#each headerRow.cells as cell (cell.id)}
								<Subscribe attrs={cell.attrs()} let:attrs props={cell.props()} let:props>
									<Table.Head {...attrs} class="text-left [&:has([role=checkbox])]:pl-3">
										{#if cell.id === 'deadline' || cell.id === 'name'}
							...
							{#each row.cells as cell (cell.id)}
								<Subscribe attrs={cell.attrs()} let:attrs>
									<Table.Cell {...attrs}>
										...
										{:else}
											<div class="px-5 text-left font-medium">
												<Render of={cell.render()} />
											</div>
										{/if}
									</Table.Cell>
